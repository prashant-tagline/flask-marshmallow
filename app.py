...
quote_schema = QueteSchema()
quotes_schema = QueteSchema(many=True, only=("id", "content"))
...
def new_quote():
    ...
    # Create new quote
    quote = Quote(content=data["content"], author=author,
                  posted_at=datetime.datetime.utcnow())
    db.session.add(quote)
    db.session.commit()
    result = quote_schema.dump(Quote.query.get(quote.id))
    return {"message": "Create new quote.", "quote": result}
